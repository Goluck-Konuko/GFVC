...
if __name__ == "__main__":
    ...
    for seq in tqdm(seqlist):
        ...
        for qp in tqdm(qplist):
            ...
            accumulated_metrics = {}
            for m in monitor.metrics:
                output_files.update({m:open(result_dir+testingdata_name+'_'+str(seq)+'_QP'+str(qp)+f'_{m}.txt','w')})
    ...
    for m in monitor.metrics:
        np.savetxt(result_dir+testingdata_name+f'_result_{m}.txt', total_result[m], fmt = '%.5f')
