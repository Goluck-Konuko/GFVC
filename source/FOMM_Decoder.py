...
if __name__ == "__main__":
    ...
    FOMM_Analysis_Model, FOMM_Synthesis_Model = load_FOMM_checkpoints(FOMM_config_path, FOMM_checkpoint_path, cpu=cpu)
    modeldir = 'FOMM' 
...
###################################################
    ...
    for frame_idx in tqdm(range(0, frames)):            
        ...
        if frame_idx in [0]:      # I-frame                      
            ...
            kp_current_jocobi=torch.Tensor(kp_jocobi).to(device)          
            ...
            prediction = make_FOMM_prediction(reference, kp_current, kp_reference, FOMM_Synthesis_Model, cpu=cpu) #######################
