...
class FastEnc:
    def __init__(self, model_order=0, eof=2) -> None:
        self.eof = eof
        # self.ppm_model = PpmModel(model_order, self.eof+1, self.eof)
        # self.dec_ppm_model = PpmModel(model_order, self.eof+1, self.eof)
    
    def exp_golomb_coder(self,datares,outputfile, MODEL_ORDER=0):
        # Must be at least -1 and match ppm-decompress.py. Warning: Exponential memory usage at O(257^n).
        with contextlib.closing(BitOutputStream(open(outputfile, "wb"))) as bitout:  #arithmeticcoding.
            enc = ArithmeticEncoder(32, bitout) #########arithmeticcoding.
            #print(enc)
            ppm_model = PpmModel(MODEL_ORDER, 3, 2)  ##########ppmmodel.
            #print(model)
            history = []
            ...
            for ii in symbollist:
                ...
                encode_symbol(ppm_model, history, i_number, enc)

                ppm_model.increment_contexts(history, i_number)
                if ppm_model.model_order >= 1:
                    if len(history) == ppm_model.model_order:
                        history.pop()
                    history.insert(0, i_number) ###########
            encode_symbol(ppm_model, history, self.eof, enc)  # EOF ##########
            enc.finish()  #        
