...
class DenseMotionNetwork(nn.Module):
    ...
    def create_sparse_motions(self, source_image, heatmap_driving, heatmap_source):
        ...
        heatmap_source_lt = heatmap_source_lt.data.cpu().numpy()         
        ...
        heatmap_driving_lt = heatmap_driving_lt.data.cpu().numpy()         
        ...
        for tensorchannel in range(0,bs):
            ...
            GFflow.append(flow)
        if source_image.get_device()<1:
            tmp_flow=torch.Tensor(np.array(GFflow))
        else:
            tmp_flow=torch.Tensor(np.array(GFflow)).to(source_image.get_device())   
        return tmp_flow    
