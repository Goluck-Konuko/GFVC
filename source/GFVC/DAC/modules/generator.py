...
#             mask = self.ref_fusion_net(inp)
#             def_ref_fts = mask*def_ref_fts + (1-mask)*prev_anim_fts
...
#                 prev_anim_fts = self.reference_ft_encoder(output_dict[f'prediction_{idx-1}'])
#                 prev_motion_pred_params = { 
#                         'reference_frame':output_dict[f'prediction_{idx-1}'],
...
#     def generate_animation(self, reference_frame: torch.Tensor, **params) -> torch.Tensor: 
...
#         if self.ref_fusion_net:
#             if 'prev_animation' in params:
#                 #Extract features from previously animated frame
#                 prev_anim_fts = self.reference_ft_encoder(params['prev_animation'])
#                 #deform the features using a predicted warping field
#                 prev_motion_pred_params = { 
#                         'reference_frame': params['prev_animation'],
#                         'reference_frame_features': prev_anim_fts,
#                         'kp_reference':params['prev_kp_target'],
#                         'kp_target':params['kp_target']
#                     }
#                 def_prev_anim_fts, _ = self.motion_prediction_and_compensation(**prev_motion_pred_params)
#                 #predict a mask to combine the two features
#             else:
#                 def_prev_anim_fts  = torch.zeros_like(def_ref_fts)
#             cond_ref_features = torch.cat((def_ref_fts,def_prev_anim_fts), dim=1)
#             def_ref_fts = self.ref_fusion_net(cond_ref_features)
